...
test('Ergebniskarte öffnet Detailansicht und Zurück behält Suchliste', async ({ page }) => {
  ...
  await expect(page.getByLabel('Pikachu Artwork')).toBeVisible();
  ...
  await expect(page.getByText('Gewicht')).toBeVisible();
...
test('Detailansicht zeigt nur den bereinigten Kerninhalt', async ({ page }) => {
  await routeTextSearchPikachu(page);
  await page.goto('/');
